...
# class SettlingProbe(PROB_OBJ):
    # def __init__(self, config, mcu_probe):
        ...
        # if config.has_section('probe'):
            ...
            # self.stow_on_each_sample = pconfig.getboolean('deactivate_on_each_sample', True)
        # else:
            # raise self.printer.config_error("SettlingProbe.__init__: A probe is needed for %s"
                                             # % (self.name))   
        # # Register homing event handlers
        # self.printer.register_event_handler("homing:homing_move_begin",
                                            # PROB_OBJ._handle_homing_move_begin)
        # self.printer.register_event_handler("homing:homing_move_end",
                                            # PROB_OBJ._handle_homing_move_end)
        # self.printer.register_event_handler("homing:home_rails_begin",
                                            # PROB_OBJ._handle_home_rails_begin)
        # self.printer.register_event_handler("homing:home_rails_end",
                                            # PROB_OBJ._handle_home_rails_end)
        # self.printer.register_event_handler("gcode:command_error",
                                            # PROB_OBJ._handle_command_error)
        # # Register PROBE/QUERY_PROBE commands
        # self.gcode = self.printer.lookup_object('gcode')
        # self.gcode.register_command('PROBE', PROB_OBJ.cmd_PROBE,
                                    # desc=PROB_OBJ.cmd_PROBE_help)
        # self.gcode.register_command('QUERY_PROBE', PROB_OBJ.cmd_QUERY_PROBE,
                                    # desc=PROB_OBJ.cmd_QUERY_PROBE_help)
        # self.gcode.register_command('PROBE_CALIBRATE', PROB_OBJ.cmd_PROBE_CALIBRATE,
                                    # desc=PROB_OBJ.cmd_PROBE_CALIBRATE_help)
        # self.gcode.register_command('PROBE_ACCURACY', PROB_OBJ.cmd_PROBE_ACCURACY,
                                    # desc=PROB_OBJ.cmd_PROBE_ACCURACY_help)
        # self.gcode.register_command('Z_OFFSET_APPLY_PROBE',
                                    # PROB_OBJ.cmd_Z_OFFSET_APPLY_PROBE,
                                    # desc=PROB_OBJ.cmd_Z_OFFSET_APPLY_PROBE_help)                          
        # self._log_config()
    # def run_probe(self, gcmd):
        # if self.settling_sample:
            ...
            # speed = gcmd.get_float("PROBE_SPEED", self, speed, above=0.)
            # self._probe(speed)
        # return PROB_OBJ.run_probe(self, gcmd)
    ...
    # def cmd_PROBE_ACCURACY(self, gcmd):
        # if self.settling_sample:
            ...
            # speed = gcmd.get_float("PROBE_SPEED", self.speed, above=0.)
            # self._probe(speed)
        # return PROB_OBJ.cmd_PROBE_ACCURACY(self, gcmd)
    ...
    # def _log_config(self):
        ...
        # #logging.debug(
                        ...
                        # "\nself.stow_on_each_sample = %s;"
                        # #"\nself.probe = %s;"
                        # "\nPROB_OBJ (probe.PrinterProbe) = %s;"
                        # "\nself.mcu_probe (probe.ProbeEndstopWrapper) = %s;"
                        # "\nprobe.py Class ProbeEndstopWrapper"
                        # "\nvariables are as follows:"
                        # "\nself.mcu_probe.position_endstop (z_offset) = %s;"
                        # "\nself.mcu_probe.stow_on_each_sample = %s;"                        
                        # "\nself.mcu_probe.activate_gcode = %s;"
                        # "\nself.mcu_probe.deactivate_gcode = %s;"
                        # "\nself.mcu_probe.mcu_endstop (Z endstop OBJ) = %s;"
                        # "\nself.mcu_probe.get_mcu = %s;"
                        # "\nself.mcu_probe.add_stepper = %s;"
                        # "\nself.mcu_probe.get_steppers = %s;"
                        # "\nself.mcu_probe.home_start = %s;"
...
# def load_config(config):
    ...
    # return SettlingProbe(config, probe_obj.mcu_probe)